...
class ContentInjector {
    constructor() {
        ...
        this.expander = window.resumeExpander; // НОВОЕ: модуль разворачивания блоков
    ...
    injectAnalysisBlock() {
        ...
        // НОВОЕ: разворачиваем скрытые блоки для полного парсинга
        if (this.expander) {
            this.expander.expandAll().then(result => {
                console.log('📋 Блоки развёрнуты:', result);
            }).catch(e => {
                console.warn('⚠️ Ошибка разворачивания блоков:', e);
            });
        }
    }
 ...
 */
    ...
    async checkCache(forceShow = false) {
        ...
        const cvData = await this.parser.extractAll();
        const select = this.vacancySelect;
    ...
    async analyze(forceNew = false) {
        ...
        try {
            ...
            // ОБНОВЛЕНО: передаём cv_structured в API
            const cvStructured = cvData.cvStructured || null;
            const result = await this.api.analyze(vacancyId, cvData.fullText, candidateName, currentUrl, cvStructured);
    ...
    displayResult(result, isCached = false) {
        ...
        if (!analysis._hash && this.vacancySelect?.value) {
            // Используем кэшированные данные парсинга (extractAll() теперь async)
            const fullText = this._lastCvData?.fullText || '';
