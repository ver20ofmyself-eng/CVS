 ...
 */
    ...
    getHTML() {
      ...
      <div class="cvs-analyzer-widget">
        <div class="cvs-header">
          ...
          <div class="cvs-status" id="cvs-auth-status"></div>
        </div>
        ...
        <div id="cvs-result" class="cvs-result" style="display: none;">
          ...
          <div class="cvs-footer">
            ...
            <div class="cvs-actions">
              <button id="cvs-clear-btn" class="cvs-clear-btn" title="Очистить результат">🗑️</button>
              <button id="cvs-update-btn" class="cvs-update-btn" title="Обновить анализ">🔄</button>
              <button id="cvs-compare-btn" class="cvs-compare-btn" title="Сравнить с предыдущими">📊</button>
            </div>
          </div>
        </div>
    ...
    async analyze(forceNew = false) {
        ...
        try {
            ...
            const result = await this.api.analyze(vacancyId, cvData.fullText);
    ...
    updateResultContent(analysis, isCached) {
        ...
        if (scoreBadge) {
            ...
            if (score >= 8) scoreClass = 'cvs-score-high';
            else if (score >= 5) scoreClass = 'cvs-score-medium';
    ...
    showQuickSwitch() {
        chrome.storage.local.get(['recent_analyses'], (items) => {
            ...
            recent.slice(0, 5).forEach((item, index) => {
                ...
                option.innerHTML = `
                <span class="cvs-quick-switch-score">${item.score || '?'}/10</span>
