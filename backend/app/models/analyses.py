...
class Analysis(Base):
    ...
    # ── Идентификация анализа ──────────────────────────────────────────────────
    candidate_name = Column(String(255), nullable=True)   # "Иванов Иван"
    source_url = Column(Text, nullable=True)              # URL страницы резюме (hh.ru/resume/...)
    analysis_number = Column(Integer, nullable=True)      # Порядковый номер анализа по данному source_url

    # ── Исходные данные ────────────────────────────────────────────────────────
    cv_text = Column(Text, nullable=False)
    cv_text_preview = Column(String(500))
    cv_structured = Column(JSON, nullable=True)   # НОВОЕ: структурированные данные резюме (9 блоков)
