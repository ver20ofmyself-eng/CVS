...
async def analyze_cv(
    ...
    # 4.1 Определяем cv_text для AI: структурированный (если есть) или сырой
    if request.cv_structured:
        cv_for_ai = ai_service.format_cv_for_ai(request.cv_structured)
        cv_for_db = cv_for_ai if cv_for_ai else request.cv_text
        logger.info("📊 Используем структурированные данные (cv_structured), %d блоков",
                     sum(1 for k in ["cover_letter", "desired_position", "experience", "skills", "about", "education"]
                         if request.cv_structured.get(k)))
    else:
    ...
    # 5. AI-анализ
    try:
        analysis_result = await ai_service.analyze_cv(vacancy_dict, cv_for_ai, db=db, user_id=current_user.id)
        ...
        # 8. Сохраняем
        cv_preview = cv_for_db[:200] + ("..." if len(cv_for_db) > 200 else "")
        db_analysis = Analysis(
            ...
            cv_structured=request.cv_structured,  # НОВОЕ: сохраняем структурированные данные
