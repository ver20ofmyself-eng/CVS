...
class AIService:
    ...
    def _mock_analysis(self, vacancy: Dict[str, Any], cv_text: str) -> Dict[str, Any]:
        ...
        score = round((matched_w / total_w * 100), 0) if total_w else 50
        if   experience_years >= 5: score = min(100, score + 10); exp_txt = "Senior"
        elif experience_years >= 3: score = min(100, score + 5);  exp_txt = "Middle"
        ...
        if   score >= 85: rec = "Пригласить на интервью";        ns = "Провести техническое интервью"
        elif score >= 70: rec = "Рекомендуется к рассмотрению";  ns = "Сравнить с другими кандидатами"
        elif score >= 50: rec = "Рассмотреть с оговорками";      ns = "Уточнить недостающие навыки"
        elif score >= 25: rec = "Не рекомендуется";              ns = "Рассмотреть только при нехватке кандидатов"
        ...
        return dict(
            score=score,
            summary=f"{exp_txt}. Навыки: {len(matched)}/{len(required_skills)}. Совпадение: {score}%. [MOCK]",
            ...
            questions_for_interview=["Уточните опыт работы с ключевыми технологиями"],
    ...
    # ── Форматирование структурированного CV для AI ─────────────────────────
    @staticmethod
    def format_cv_for_ai(cv_structured: dict) -> str:
        """
        Форматирует структурированные данные CV в текст с заголовками блоков.
        Используется 6 блоков для AI: cover_letter, desired_position, experience,
        skills, about, education.
        """
        parts = []

        # Сопроводительное письмо
        cover = cv_structured.get("cover_letter") or {}
        if cover.get("text"):
            parts.append(f"## Сопроводительное письмо:\n{cover['text']}")

        # Желаемая должность
        pos = cv_structured.get("desired_position") or {}
        if pos:
            pos_lines = []
            if pos.get("titles"):
                pos_lines.append(f"Должности: {', '.join(pos['titles'])}")
            if pos.get("specializations"):
                pos_lines.append(f"Специализации: {', '.join(pos['specializations'])}")
            if pos.get("salary"):
        ...
        exp = cv_structured.get("experience") or {}
        if exp:
            ...
            positions = (exp.get("positions") or [])[:5]
            pos_texts = []
            for p in positions:
        ...
        # Навыки
        skills = cv_structured.get("skills") or {}
        if skills:
            skill_lines = []
        ...
        # О себе
        about = cv_structured.get("about") or {}
        if about.get("text"):
            parts.append(f"## О себе:\n{about['text']}")

        # Образование
        edu = cv_structured.get("education") or {}
        if edu:
            edu_lines = []
            if edu.get("level"):
                edu_lines.append(f"Уровень: {edu['level']}")
            for inst in edu.get("institutions") or []:
                line = inst.get("name", "?")
                if inst.get("faculty"):
                    line += f", {inst['faculty']}"
