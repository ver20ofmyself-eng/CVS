...
const loadPayments = async () => {
  ...
  try {
    const response = await api.get('/tariffs/payments/history')
...
const purchaseTariff = async () => {
  ...
  try {
    const response = await api.post(`/tariffs/purchase/${selectedTariff.value}`)
    ...
    if (response.data.success) {
      ...
      toast.success(response.data.message || 'Тариф успешно активирован!')
    }
  } catch (err) {
    console.error('Ошибка покупки:', err)
    toast.error(err.response?.data?.detail || 'Не удалось совершить покупку')
  }
}
