        ...
        >
          ...
          <div class="analysis-header">
            <div class="analysis-title">
              <h3>{{ analysis.vacancy_title || 'Вакансия удалена' }}</h3>
...
import { format, formatDistance, subDays, startOfDay, endOfDay } from 'date-fns'
...
const applyPeriodFilter = () => {
  ...
  switch (filters.period) {
    case 'today':
      filters.dateFrom = format(startOfDay(today), 'yyyy-MM-dd')
      filters.dateTo = format(endOfDay(today), 'yyyy-MM-dd')
      break
    case 'week':
      filters.dateFrom = format(subDays(today, 7), 'yyyy-MM-dd')
      filters.dateTo = format(today, 'yyyy-MM-dd')
      break
    case 'month':
      filters.dateFrom = format(subDays(today, 30), 'yyyy-MM-dd')
      filters.dateTo = format(today, 'yyyy-MM-dd')
      break
    case 'custom':
...
const exportData = async (format) => {
  try {
    ...
    if (format === 'csv') {
      ...
      filename = `analyses_${format(new Date(), 'yyyyMMdd_HHmm')}.csv`
      type = 'text/csv'
    } else {
      content = JSON.stringify(response.data, null, 2)
      filename = `analyses_${format(new Date(), 'yyyyMMdd_HHmm')}.json`
